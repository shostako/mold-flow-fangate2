...
def test_fan_runner_builder_records_the_gate_orifice_as_the_nominal_marker():
    """The fan-runner plate's injection point is the hot-runner gate disc on
    the sprue axis: the marker is the configured Φ (gate_d), centred on the
    axis — and the snap fallback (mesh coarser than the orifice) clears it so
    the marker shows the cell the solver actually uses."""
    from core import FanRunnerPlateConfig, build_fan_runner_plate_geometry
    ...
    assert vr == pytest.approx(cfg.gate_d_mm / 2.0)
    groups = gate_groups_mm(g)
    assert len(groups) == 1
    assert groups[0][2] == pytest.approx(vr)
    # coarse mesh: the nearest-cell snap draws the real injection cell
    g = build_fan_runner_plate_geometry(FanRunnerPlateConfig(cell_size_mm=18.0))

...
def test_the_shot_volume_defaults_to_the_cavity_volume_and_follows_the_geometry():
    ...
    # (kept ≥ the 300 mm runner width so the geometry stays valid)
    ...
    _width(at).set_value(302.26)  # back to the drawing → auto tracker returns to v0
    at.run()
...
def test_an_edited_shot_volume_survives_toggling_the_analysis_off_and_on():
    ...
    assert at.number_input(key="two_phase_shot_volume").value == v0
    assert not _edited(at)
    _width(at).set_value(400.0)  # ≥ the 300 mm runner width so the geometry stays valid
    at.run()
    assert (
        at.number_input(key="two_phase_shot_volume").value
        == at.session_state["mfs_shot_volume_auto"]
...
def test_a_geometry_error_leaves_the_output_widgets_and_the_version_footer(monkeypatch):
    ...
    monkeypatch.setattr(core, "build_fan_runner_plate_geometry", explode)
    at = _app(fast=False)
    assert not at.exception

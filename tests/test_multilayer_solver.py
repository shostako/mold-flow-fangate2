...
def _default_geometry(**overrides) -> Geometry:
    """Plate + runner + sprue demo cavity standing in for sim's film-gate
    fixture (the ported core has no gate-specific builder yet)."""
    base = dict(
        plate_w_mm=120.0,
        plate_h_mm=80.0,
        plate_thk_mm=2.0,
    ...
    return build_demo_geometry(**base)


def _stepped_plate_geometry(
    lower_thk_mm: float, upper_thk_mm: float, split_height_mm: float, **overrides
) -> Geometry:
    """Demo cavity whose plate is ``lower_thk_mm`` on the gate side (the
    ``split_height_mm`` rows nearest the runner) and ``upper_thk_mm`` beyond."""
    g = _default_geometry(plate_thk_mm=upper_thk_mm, **overrides)
    plate = g.mask & np.isclose(g.thickness_mm, upper_thk_mm)
    rows = np.where(plate.any(axis=1))[0]
    gate_side_end = rows.max() + 1  # plate rows run up to the runner (higher iy)
    n_split = int(round(split_height_mm / g.cell_size_mm))
    lower = plate.copy()
    lower[: gate_side_end - n_split, :] = False
    g.thickness_mm[lower] = lower_thk_mm
    return g
...
def test_short_shot_on_thin_plate_high_mold_threshold() -> None:
    ...
    """
    g = _stepped_plate_geometry(lower_thk_mm=0.35, upper_thk_mm=0.50, split_height_mm=20.0)
    db = MaterialDB()
    r = MultilayerHeleShawSolver(
        geometry=g,
        material=db["PP"],
...
def _thin_plate_geometry() -> Geometry:

...
def _fan_runner() -> Geometry:
    """Small fan-runner plate standing in for sim's stepped film-gate fixture:
    rim 0.35 / body 0.50 is the two-band plate, only the body compresses, the
    runner (0.3 → 2.0 ramp, round end, gate) is the fixed block."""
    cfg = FanRunnerPlateConfig(
        ...
        runner_edge_flat_mm=1.0,
        runner_ramp_end_mm=4.0,
        gate_d_mm=3.0,
        cell_size_mm=1.0,
        pad_mm=3.0,
    )
    return build_fan_runner_plate_geometry(cfg)
